...
交易是由外部擁有帳戶發起的簽名消息，由以太坊網路傳輸，並在以太坊區塊鏈上進行記錄（挖掘）。在這個基本定義背後，有很多令人驚訝和著迷的細節。看待交易的另一種方式是，它們是唯一可觸發狀態更改或導致合約在EVM中執行的東西。以太坊是一個全球的單實例狀態機器，交易是唯一可以讓狀態機“運動”，改變狀態的東西。合約不會自行運行。以太坊不會在後臺運行。一切都始於交易。

在本節中，我們將剖析交易，展示它們的工作方式，並瞭解詳細資訊。
