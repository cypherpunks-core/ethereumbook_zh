...
現今存在各式各樣基於乙太坊的網路，它們雖然大程度符合以太坊黃皮書 (Ethereum Yello Pape) 的正式規範，有些人可能會假設它們彼此間能互相操作，但這樣的假設並不成立。
...
在本節中，我們將看看兩個最常見的乙太坊客戶端，Geth 和 Parity。我們將學習如何使用它們的客戶端去啟動節點(node)，並且探索它們的命令列(command-line options) 操作方式以及應用程式編程接口（API）。
