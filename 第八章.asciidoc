...
合約繼承使我們能夠以實現模組化，可擴展性和重用的方式編寫我們的合約。我們從簡單的合約開始，實現最通用的功能，然後通過在更具體的合約中繼承這些功能來擴展它們。
...
通過繼承+mortal+，繼而繼承+owned+，+Faucet+合約現在具有構造函數和銷燬函數以及定義的owner。這些功能與+Faucet+中的功能相同，但現在我們可以在其他合約中重用這些功能而無需再次寫入它們。程式碼重用和模組化使我們的程式碼更清晰，更易於閱讀，並且更易於審計。
