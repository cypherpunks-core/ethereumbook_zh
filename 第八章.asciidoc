...
pure:: 純(pure)函數不讀寫任何變量。它只能對參數進行操作並返回數據，而不涉及任何儲存的數據。純函數旨在鼓勵沒有副作用或狀態的宣告式編程。
...
在修飾函數內部，可以訪問被修飾的函數的的所有可見符號（變量和參數）。在這種情況下，我們可以訪問在合約中宣告的+owner+變量。但是，反過來並不正確：你無法訪問修飾函數中的任何變量。
...
如你所見，+mortal+ 合約可以使用在+owned+中定義的+ownOwner+函數修飾器。它間接地也使用+owner+ address變量和+owned+中定義的構造函數。繼承使每個合約變得更簡單，並專注於其類的特定功能，使我們能夠以模組化的方式管理細節。
