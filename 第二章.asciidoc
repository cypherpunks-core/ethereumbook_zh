...
EVM 是一個全域單例 (global singleton) ，這意味著它的運作方式就好像它是一個全域的單一實體計算機，無處不在。以太坊網路上的每個節點運行 EVM 副本於本地，用以驗證合約執行，而以太坊區塊鏈上，記錄著這台世界計算機處理的交易以及智能合約上的狀態 (State) 之變化紀錄。更多更棒的細節我們可以在[link:https://github.com/cypherpunks-core/ethereumbook_zh/blob/master/%E7%AC%AC%E5%8D%81%E5%9B%9B%E7%AB%A0.asciidoc[以太坊虛擬機]]這一章看到。
