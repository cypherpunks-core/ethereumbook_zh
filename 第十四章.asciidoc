...
EVM機器語言分為特定的指令集組，例如算術運算，邏輯和比較運算，控制流，系統調用，堆棧操作和儲存器操作。除典型的 Bytecode 操作外，EVM還必須管理賬戶資訊（即地址和餘額），當前gas價格和區塊資訊。
...
World State:: 160位地址識別碼和賬戶狀態之間的映射，在不可變的_Merkle Patricia Tree_資料結構中維護。
...
Storage State:: 在EVM上運行時維護的賬戶特定狀態資訊。
