...
以太坊協議和操作的核心是以太坊虛擬機，簡稱 EVM。 正如您可能從名稱中猜到的那樣，它是一個計算引擎，與 Microsoft .NET Framework的虛擬機沒有太大差別，它與那些可以將程式語言編譯成 bytecode 的直譯器也沒有太大差別 (如Java）。 在本章中，我們將詳細介紹EVM，包括其在以太坊狀態更新的上下文中的指令集，結構和操作。

[[evm_description]]
=== 什麼是以太坊虛擬機 (EVM) ？
EVM是以太坊的一部分，負責處理智能合約的部署和執行。實際上，從一個EOA到另一個EOA的簡單價值轉移交易不需要涉及它，但其他一切都將涉及由EVM計算的狀態更新。在較高的層次上，在以太坊區塊鏈上運行的EVM可以被認為是包含數百萬個可執行對象的全局分散計算機，每個可執行對像都有自己的永久數據存儲。

EVM是一個準圖靈完備的狀態機; “準”因為所有執行過程都限於有限數量的計算步驟，可用於任何給定智能合約執行的可用氣體量。因此，停止問題“解決”（所有程序執行將停止）以及執行可能（意外或惡意）永遠運行的情況，從而避免以太坊平台完全停止。

EVM具有基於堆棧的體系結構，將所有內存中的值存儲在堆棧中。它的字大小為256位（主要用於簡化原生散列和橢圓曲線操作），並具有多個可尋址的數據組件：

[role="pagebreak-before"]
* 一個不可變的程序代碼ROM，加載了要執行的智能合約的字節碼
* 易失性存儲器，每個位置都明確初始化為零
...
[[evm_architecture]]
.以太坊虛擬機架構及執行脈落
image::images/evm-architecture.png["以太坊虛擬機架構及執行脈落"]
...
然而，Java VM（JVM）規範的某些方面確實包含與EVM的相似之處。從高級概述來看，JVM旨在提供與底層主機作業系統或硬體無關的運行時環境，從而實現各種系統的兼容性。在JVM上運行的高級程式語言（如Java或Scala）被編譯到相應的指令集 Bytecode 中。這與編譯要在EVM上運行的Solidity源檔案相當。

[[evm_bytecode_overview]]
==== EVM機器語言（ Bytecode 操作）

EVM機器語言分為特定的指令集組，例如算術運算，邏輯和比較運算，控制流，系統調用，堆棧操作和儲存器操作。除典型的 Bytecode 操作外，EVM還必須管理帳戶資訊（即地址和餘額），當前gas價格和區塊資訊。

[[common_stack_opcodes]]
通用堆棧操作:: 堆棧和記憶體管理的操作碼指令：
...
MSTORE是一個堆棧/記憶體操作（參見<<common_stack_opcodes>>），它將值保存到記憶體中，而CALLVALUE是一個環境操作碼（參見<<common_environment_opcodes>>），它返回正在執行的消息調用的存放值。
...
由於“PUSH4 0xffffffff”和“AND”指令是冗餘的，我們可以完全忽略它們，因為堆棧在完成後將保持不變。“DUP1”指令複製堆棧上的1^st^項，這是函數識別碼。下一條指令“PUSH4 0x2e1a7d4d”將抽取（uint256）函數的計算函數識別碼推送到堆棧。堆棧現在看起來如下：
...
下一條指令“EQ”彈出堆棧的前兩項並對它們進行比較。這是調度程式完成其主要工作的地方：它比較交易的msg.data欄位中發送的函數識別碼是否與withdraw（uint256）匹配。如果它們相等，則EQ將1推入堆棧，這最終將用於跳轉到fallback函數。否則，EQ將0推入堆棧。
