...
[[evm_chapter]]
== 什麼是以太坊虛擬機 (EVM) ? 
實際處理內部狀態和計算的協議部分稱為以太坊虛擬機（EVM）。從實際角度來看，EVM可以被認為是包含數百萬個物件的大型去中心化計算機。 EVM是以太坊的一部分，負責處理智能合約的部署和執行。實際上，從一個EOA到另一個EOA的簡單價值轉移交易不需要涉及它，但其他一切都將涉及由EVM計算的狀態更新。在較高的層次上，在以太坊區塊鏈上運行的EVM可以被認為是包含數百萬個可執行對象的全局分散計算機，每個可執行對像都有自己的永久數據存儲。
