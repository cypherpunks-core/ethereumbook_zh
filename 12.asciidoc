...
在本章中，我們將討論預言機 oracles，它是可以為以太坊智能合約提供外部數據源的系統。華人區塊鏈裡翻譯 oracle 為預言機；“神諭” Oracle一詞來自希臘神話，它提到了一個與神靈交流的人，他們可以看到未來的願景。 在區塊鏈的上下文中，oracle 是一個回答問題的系統，它可以回答來自於以太坊之外問題。 理想情況下，oracles 是不可靠 (trystless) 的系統，按造中心化原則 (decentralized)，這意味著它們不需要被信任。
