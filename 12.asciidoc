...
在本章中，我們將討論oracles，它是可以為以太坊智能合約提供外部數據源的系統。 “神諭”一詞來自希臘神話，它提到了一個與神靈交流的人，他們可以看到未來的願景。 在區塊鏈的上下文中，oracle是一個可以回答以太坊外部問題的系統。 理想情況下，oracles是無信任的系統，這意味著它們不需要被信任，因為它們是按照分散的原則運行的。

=== 為什麼預言機是剛需

以太坊平台的一個關鍵組件是以太坊虛擬機，它能夠在分散網絡中的任何節點上執行程序並更新受共識規則約束的以太坊狀態。 為了保持共識，EVM執行必須完全確定，並且僅基於以太坊狀態和簽名事務的共享上下文。 這有兩個特別重要的後果：第一個是EVM和智能合約沒有內在的隨機性來源; 第二，外部數據只能作為交易的數據有效載荷引入。
